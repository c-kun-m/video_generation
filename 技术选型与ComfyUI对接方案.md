...
状态：技术建议，尚未安装运行依赖、锁定版本或接通真实 ComfyUI。用户已明确采用 LangChain 做 Agent，前端可使用 React；以下细化对应实现方案，不代表 B0–B4 已完成。

## 1. 推荐方案

采用 **React + TypeScript 前端、Fastify 业务后端、LangChain JS 创作适配器、Temporal 长流程编排、PostgreSQL 业务存储，以及独立 Render Worker 通过 HTTP / WebSocket 调用 ComfyUI**。视频合成使用 FFmpeg。
...
| 业务 API | Node.js 24 + TypeScript + Fastify | 身份、项目、版本、审批、命令、预算、素材和状态查询 |
| Agent | LangChain JS / TypeScript | 需求整理、剧本、分镜、修改建议及结构化输出，封装成 LangChainAgentProvider |
| 持久化编排 | Temporal + TypeScript SDK | 阶段推进、等待审批、暂停/恢复、错误分流和跨进程恢复 |
| 业务数据库 | PostgreSQL 17 + pg + 版本化 SQL migrations | 不可变版本、任务/尝试、Outbox、预算账本、依赖边；沿用 G01 基线 |
| 数据合同 | JSON Schema 2020-12 + Ajv2020 | 统一 DTO、Agent 输出和 Worker 协议；从同一合同生成 TypeScript 类型 |
| 渲染适配 | Node.js Render Worker + fetch / WebSocket + SQLite journal | 领取任务、持久化回执、ComfyUI 调用、回查和文件收集 |
| 视频推理 | ComfyUI 独立 Python / PyTorch 环境 | 执行锁定的工作流、模型和自定义节点 |
| 媒体后期 | 独立 Media Worker + FFmpeg / ffprobe | 音视频探测、静态分镜预览、规格统一、字幕、混音与导出 |
| 文件存储 | StoragePort；开发本地文件，部署 S3 兼容存储 | 保存参考图、配音、原始镜头、选片和成片；数据库保存引用与摘要 |
| 工程与验证 | pnpm workspace；Vitest、Playwright、Temporal 测试工具 | 模块合同、数据库并发、恢复故障与完整用户旅程验证 |
...
Node 24 延续原稿基线；所有具体补丁版本、pnpm 版本、Python / PyTorch / CUDA 组合及制品摘要在 B0 兼容验证后锁定。原稿提到的 pnpm 11.13.0 和上游锁文件不能当作本仓库已验证的依赖。运行时状态以官方发布信息和实际锁文件为准。[Node 发布信息](https://nodejs.org/en/about/previous-releases)
...
| 原目标 | 本次处理建议 |
|---|---|
| G01、G05 的 AgentProvider / DSH | 保留 Provider 接口，首个真实实现改为 LangChainAgentProvider；DSH 留作可选适配，不作为启动依赖 |
| G04 的唯一全局编排器 | 保留 Temporal；LangChain 只在某个创作 Activity 内执行 |
| G14 的 Electron + React | 保留桌面交付范围，后续新建独立外壳或接入实际提供的桌面工程；不假定已有 Bridge |
| G02 的业务真源 | 保留 PostgreSQL 和不可变版本，Agent 的消息历史不能代替项目数据 |
...
推荐 LangChain JS：前端、业务层、Temporal 和创作适配器可共享 TypeScript 数据类型。ComfyUI 虽用 Python 实现，对外提供语言无关的网络协议，因此不会迫使业务后端改用 Python。未来出现只有 Python 库能实现的创作能力时，可新增 Python Provider 服务，保持业务合同不变。
...
仅需结构化生成时使用 LangChain 模型接口；确需查询素材等工具时使用 `createAgent`，配置 `responseFormat`。结构化输出可使用 JSON Schema，结果再次通过本项目校验器后由领域服务保存。[LangChain 结构化输出](https://docs.langchain.com/oss/javascript/langchain/structured-output)

LangChain 的 Agent 底层使用 LangGraph。这与 G04 并不冲突：允许其处理单个创作任务内部的模型/工具步骤；项目审批、GPU 派发、取消和成片推进仍由 Temporal 与领域服务负责，不另建第二套项目状态机。[LangChain 官方概览](https://docs.langchain.com/oss/javascript/langchain/overview)

建议边界：`AgentProvider.generate(spec, operationId)` / `query(operationId)`。LangChain、供应商 SDK 和模型请求都在 Activity 内，Temporal Workflow 不直接调用网络。Activity 的重试不会自动让外部副作用变成只发生一次。[Temporal Activity 定义](https://docs.temporal.io/activity-definition)

落实 G05 每任务最多 3 次模型调用时，必须把 LangChain 内部格式修复、供应商 SDK 重试和工具循环一并纳入计数与预算。持久化每次 operation_id 及结果引用；调用结果未知时进入回查/待处理，不能让多层重试重复扣费。

JSON Schema 2020-12 采用独立 Ajv2020 校验器。Fastify 的请求验证和响应序列化都要验证方言兼容性；必要时采用显式验证后的普通 JSON 序列化，避免默认编译器误解合同。面向模型供应商的 Schema 子集由适配器转换，返回结果仍按完整业务 Schema 校验。[Ajv 方言说明](https://ajv.js.org/json-schema.html)、[Fastify 验证与序列化](https://fastify.dev/docs/latest/Reference/Validation-and-Serialization/)
...
flowchart TD
    ...
    Bridge --> API[Fastify Video API / 领域服务]
    API --> DB[(PostgreSQL)]
    DB --> Outbox[Outbox 投递器]
    Outbox --> T[Temporal Workflow]
    T --> A[创作 Activity / LangChain]
    A --> Draft[校验并保存创作草稿]
    T --> Plan[校验审批与冻结计划]
    Plan --> Jobs[领域服务登记 RenderJob]
    Jobs --> DB
    R[Render Worker] -->|内部 API 领取与上报| API
    R -->|HTTP 提交 / 查询| C[ComfyUI]
    C -->|WebSocket 进度| R
    R -->|校验后上传| S[素材存储 / Artifact]
    T --> M[Media Worker / FFmpeg]
    S --> M
    M --> Final[成片候选 / 审批 / 导出]
```

对 ComfyUI 的核心调用由 Render Worker 封装。前端提交“生成镜头”领域命令；Agent 生成镜头描述；编译器把描述映射到经过测试的工作流；Worker 执行和收集。这样关闭界面、Agent 调用结束或某个进程重启，都不会丢失已登记的制作记录。
...
下面只展示提交体的构造方式；`compiledApiGraph` 等来自上述预检与 journal 流程，不是本次已实现的客户端代码。

```typescript
const requestBody = {
  prompt: compiledApiGraph,
  client_id: websocketClientId,
  extra_data: {
    video_generation: {
      attempt_id: attemptId,
      render_spec_digest: renderSpecDigest,
    },
  },
};

// 只有 journal 持久化 SUBMITTING 后才允许执行；本层禁用自动提交重试。
const response = await fetch(`${configuredComfyBaseUrl}/prompt`, {
  method: "POST",
  headers: { "Content-Type": "application/json" },
  body: JSON.stringify(requestBody),
  signal: submitAbortSignal,
});
// 后续必须校验 HTTP 状态和响应 Schema；先保存回执，再上报 ACCEPTED。
// 请求超时/响应丢失进入回查，不能直接再次执行 fetch。
```
...
```text
apps/video-desktop/             Electron 外壳与 React 工作台
apps/video-api/                 Fastify、Outbox、内部 Worker API
apps/video-workflow-worker/     Temporal Workflows / Activities
apps/video-render-worker/       ComfyUI 客户端、SQLite journal、回查
apps/video-media-worker/        TTS/对齐适配、FFmpeg、媒体任务日志
packages/video-contracts/       JSON Schema、生成类型、错误和事件
packages/video-domain/          命令、审批、预算、版本、任务与依赖
packages/video-storage/         PostgreSQL 仓储与 StoragePort
packages/video-agent-provider/  LangChainAgentProvider 与 FakeProvider
packages/video-desktop-client/ Video Client / Bridge 合同
workflows/                     版本化 API 图、bindings、Bundle 清单
deploy/video/                  Compose、版本锁与恢复脚本
```
